...
<body>
    ...
    {% block body %}{% block content %}{% load carton_tags %}{% get_cart as cart %}
        <div class="ui pink stackable menu" style="margin-top:10px;">
            ...
            {% if user.is_staff %}<a class="item" href="{% url 'posts:list' %}"><i class="announcement icon"></i> 管理 </a>{% endif %}
            {% if user.is_authenticated %}<div class="ui top right attached medium pink label" id="login_tag"><i class="sign in icon"></i>{{ request.user.username }} 已登入</div>
