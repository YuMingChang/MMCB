...
<body>
  ...
  <div class="ui fixed pink menu">
    <div class="ui container" id="MenuContainer">
      ...
      <a href="{% url 'store' %}" class="item">商店</a>
      <a href="{% url 'cart:shopping-cart-show' %}" class="item" value="{{ cart.unique_count }}">購物車</a>
      {% if user.is_authenticated %}
      <a href="{% url 'member:page' %}" class="item">會員系統</a>
      ...
      {% endif %} {% if user.is_staff %}<a href="{% url 'posts:page' %}" class="item">管理系統</a>{% endif %}
    </div>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      var cart_count = $("#MenuContainer").find('a:nth-child(3)').attr("value");
      $('#MenuContainer').find('a:nth-child(3)').text("購物車(" + cart_count + ")");
      $('.ui .item').on({
        mouseenter: function() {
